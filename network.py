...
class ResBlk(nn.Module):
    ...
    def _build_weights(self, dim_in, dim_out):
        ...
        if self.learned_sc:
            self.conv1x1x1 = nn.Conv2d(dim_in, dim_out, 1, 1, 0, bias=False)
...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512):
        ...
        )  # Padding added to fix output size - need to figure out why output img size is wrong
...
class StyleEncoder(nn.Module):
    def __init__(self, img_size=256, style_dim=64, num_domains=2, max_conv_dim=512):
        ...
        self.unshared = nn.ModuleList()
        for _ in range(num_domains):
            # 864 was hardcoded, originally dim_out, varies as img size varies
